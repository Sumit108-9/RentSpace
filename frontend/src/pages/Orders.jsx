    ...
    const fetchOrders = async () => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (data.success) { setOrders(data.orders || []); return; }
        }
      } catch (err) {}
      setOrders([
        { _id: 'ORD0012', name: '3 Seater Sofa', duration: 6, date: '10 May 2024', price: 7794, status: 'Active', icon: '🛋' },
  ...
  return (
    <div>
      ...
      {filteredOrders.map((order, idx) => (
        <div key={idx} style={{ background: '#fff', border: '0.5px solid #e8e6df', borderRadius: 12, padding: '20px 28px', display: 'flex', gap: 20, alignItems: 'center', marginBottom: 16 }}>
